...
# Build
echo "Build running"
./gradlew clean build install -Prelease.version="${RELEASE_VERSION}" || exit
...
sudo docker build \
  ...
  -t docker.pkg.github.com/queueing-systems-assistance/qsa-system-config-pack/qsa-system-config-pack:"${RELEASE_VERSION}" . || exit
sudo docker push docker.pkg.github.com/queueing-systems-assistance/qsa-system-config-pack/qsa-system-config-pack:"${RELEASE_VERSION}" || exit
